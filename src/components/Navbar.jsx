...
import API_BASE from "../config/api"; // Ensure this path is correct for your project
...
const Navbar = () => {
  ...
  /* ─── 1. PAGE TRACKING & SCROLL RESET ─── */
  useEffect(() => {
    // Reset scroll to top
    window.scrollTo({ top: 0, behavior: "instant" });
    setMenuOpen(false);

    // Send Tracking Data to PHP Backend
    fetch(`${API_BASE}/api.php/track`, {
      method: "POST",
      headers: {
  ...
  /* ─── 2. SCROLL LISTENER (FOR GLASS EFFECT) ─── */
  useEffect(() => {
  ...
  /* ─── 3. PREVENT BODY SCROLL WHEN MENU OPEN ─── */
  useEffect(() => {
  ...
  return (
    <>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.to}
                  to={link.to}
                  className={`relative px-3.5 py-2 text-[13px] font-medium rounded-lg transition-all duration-200 ${isActive
                    ? "text-white"
      ...
      {/* Drawer Panel */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-72 bg-[#0b1f3a] shadow-2xl flex flex-col transition-transform duration-300 ease-in-out md:hidden ${menuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        {/* Drawer Header */}
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.to}
                to={link.to}
                className={`flex items-center justify-between px-4 py-3 rounded-xl text-[13px] font-medium transition ${isActive
